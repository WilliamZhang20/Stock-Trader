...
using a quadratic program in the `OSQP` solver from CVXPY. Rather than a raw sample mean (a notoriously noisy input that makes textbook Markowitz overfit), $\mu$ is a **Black-Litterman posterior** that mixes:

1. absolute EWMA views ($P = I$), and
2. **relative residual-mean-reversion views** between PCA-cluster peers ($P_k = e_i - \beta e_j$), with $\Omega_{kk}$ inflated when the relationship is unstable.

$\Sigma$ is an **Iterated EWMA (IEWMA)** forecast (EWMA vols, then EWMA correlation of vol-standardized returns — Engle / Barratt-Boyd). Portfolio risk is stabilized by an explicit **volatility target**. Trading costs enter the QP as an **asset-specific L1** $\sum_i c_i |w_i - w_{\mathrm{prev},i}|$ (half-spread floor + vol load), which creates a genuine **no-trade region** so the book can rebalance **daily** without daily churn — the same $c_i$ is charged in the equity-curve backtest. This replaces an earlier fragile HMM regime-switching risk model and the v6 weekly-rebalance workaround. Ablations: [trials/improve_mv.py](trials/improve_mv.py); robustness / selection null: [trials/e8_robustness.md](trials/e8_robustness.md), [trials/e9_bootstrap.md](trials/e9_bootstrap.md).
...
- **Transaction costs.** Mean-variance charges asset-specific $c_i$ (base half-spread + vol load) on $|\Delta w_i|$ in *both* the optimizer and the backtest — inducing a no-trade region. CVaR still uses a scalar `COST_BPS` (default 10 bps) on L1 turnover. E8 reports gross vs net side by side so the cost drag is explicit.
- **Overfitting.** The strategies carry many hand-tuned constants, and several configurations were tried on the same data. E8 (a) scores each strategy on consecutive, non-overlapping out-of-sample sub-windows and (b) reports a **Deflated Sharpe Ratio** that discounts for having tried ~N configurations. **E9** goes further: it re-runs the full universe/config search on block-bootstrap return panels and compares the real best Sharpe to that null distribution of *selected* Sharpes — answering whether the result could arise from the amount of experimentation performed.
